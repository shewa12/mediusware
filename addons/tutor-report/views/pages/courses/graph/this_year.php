...
$enrolledQuery = $wpdb->get_results( "SELECT SUM(meta.meta_value) as total_sales, MONTHNAME(posts.post_date) as month_name from {$wpdb->posts} AS posts
    LEFT JOIN {$wpdb->postmeta} AS meta2 ON posts.ID = meta2.post_id
    LEFT JOIN {$wpdb->postmeta} AS meta ON posts.ID = meta.post_id
    WHERE posts.post_type = 'shop_order'
    AND meta.meta_key = '_order_total'
    AND meta2.meta_key = '_tutor_order_for_course_id_{$current_id}'
    AND YEAR(posts.post_date) = {$currentYear} 
