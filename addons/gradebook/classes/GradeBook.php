...
class GradeBook{
	...
	public function add_new_gradebook(){
		...
		$required_fields = apply_filters('tutor_gradebook_required_fields', array(
			...
			'grade_for'             => __('Grade for field is required', 'tutor'),
		));
		...
		$data = array(
			...
			'grade_for'             => sanitize_text_field(tutils()->array_get('grade_for', $_POST)),
	...
	public function quiz_attempt_ended($attempt_id){
		...
		WHERE grade_for = 'quiz' 
		AND percent_from <= {$earned_percentage} 
		...
		$gradebook_data = array(
			...
			'earn_grade_point'   => $gradebook->grade_point,
		...
		if ($gradebook_result){
			$gradebook_result_id = $gradebook->gradebook_result_id;
			//Update Gradebook Result
			unset($gradebook_data['generate_date']);
			$wpdb->update($wpdb->tutor_gradebooks_results, $gradebook_data, array('gradebook_result_id' => $gradebook->gradebook_result_id ) );
		}else{
